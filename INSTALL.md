...
V1.0.0 的支持目标为 Windows 11 x64 / Codex / SWD。当前交付仍为候选，尚未完成干净 Windows、新 Codex 桌面任务及新二进制真机验收；正式状态以对应版本的 Release 说明为准。JTAG、其他客户端和 ARM64 未在本次发布范围内。

## 用户自行准备

- Codex，以及可运行 `codex plugin` 的 Codex CLI。CLI 具体安装方式由用户选择；本工具不自动下载或安装 Codex，也不要求必须使用 npm 版本。
- 需要操作硬件时，自行安装兼容的 SEGGER J-Link 软件及探针所需驱动，并准备目标工程、ELF 和实际器件标识。当前已验证的 J-Link 基线是 6.98a；不能直接假定任意新版本兼容。
...
1. 从可信的仓库 Release 或明确标为候选的交付位置取得 `jlink-mcp-V1.0.0-windows-x64.zip` 与 `.zip.sha256`。使用 `Get-FileHash <ZIP路径> -Algorithm SHA256` 与可信发布页的值核对；同来源的可修改摘要只能帮助检错，不能独立认证发布者。本版未进行代码签名。
...
4. 安装结果应包含版本 `1.0.0`、deployment 路径和 `segger_managed: false`。打开新的 Codex 任务，确认只出现本插件固定的六工具。
...
jlink-mcp/
  ...
  deployments/1.0.0-<16位摘要前缀>/     成套 EXE、插件、marketplace 和安装资料
...
需要回退时，关闭本工具会话，然后用保留的上一份完整发布包运行同一安装器。升级或回退都不会修改工程配置、`.jlink-mcp/captures`、其他插件或 SEGGER 文件。
