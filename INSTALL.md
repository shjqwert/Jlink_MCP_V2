...
1. 从可信的仓库 Release 取得 `jlink-mcp-V1.1.4-windows-x64.zip` 与 `.zip.sha256`。使用 `Get-FileHash <ZIP路径> -Algorithm SHA256` 与可信发布页的值核对；同来源的可修改摘要只能帮助检错，不能独立认证发布者。本版未进行代码签名。
...
4. 安装结果应包含版本 `1.1.4`、deployment 路径和 `segger_managed: false`。打开新的 Codex 任务，确认只出现本插件固定的六工具。
...
jlink-mcp/
  ...
  deployments/1.1.4-<16位摘要前缀>/     成套 EXE、插件、marketplace 和安装资料
