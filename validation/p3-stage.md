...
P3 的 4.1–4.8 已完成。冻结生产链路在 S32K144、SWD 4000 kHz 和 J-Link 6.98a 上完成一次连续 10×32-bit、1 kHz、300 秒 HSS：活动窗口内只允许串行 RAM 写入并读回恢复，固定时长后自动 Stop、尾排空、原子发布完成文件，最终变量原值和 CPU running 状态均已确认。该次运行还记录了已废弃的父进程续行与跨 MCP 同键恢复行为；这两项只保留为历史事实，不再属于当前设计或验收结论。本记录只是 P3 实现检查点，不代表 5.6、5.7、JTAG 真机或发布完成。
...
| 生命周期说明 | 本次历史运行曾验证父进程续行和跨 MCP 同键恢复；当前合同已取消该能力，修订后的脚本保持单一 MCP/Worker 生命周期 |
...
- 本次历史运行在 Start 返回 `running` 后曾终止原 MCP 并由新 MCP 取得同一 ID；该步骤及其结论已被当前“Worker 生命周期绑定 MCP/Codex”设计废弃，修订后的脚本不再终止父 MCP 或接管旧 Worker。
...
- 历史运行在完成边界上曾通过新 MCP 重新连接目标；当前脚本改为由同一 MCP 确认最终内存 `78563412`、目标 `connected/running` 并 disconnect。既有变量恢复、CPU 安全状态和 J-Link 相关进程数为 0 的硬件事实不因取消接管能力而改变。
...
4.8 是跨组件阶段 smoke，不替代 4.1–4.7 的唯一主要测试。它证明冻结输入下的实际 HSS Start、写入交错、300 秒固定时长、自动 Stop、尾排空、质量事实和完成文件组合链路；不再证明父进程续行、跨 MCP 接管或旧 key 恢复。它不证明 JTAG 真机、所有异常注入、查询视图、Codex 客户端或发布门禁。DLL、探针、目标、接口/速度、OUT、夹具、HSS ABI、正常采集调度、Capture Store 格式、质量算法、IPC/Schema 或 smoke 驱动任一相关变化时，本阶段对应证据失效；仅所有权退出/关闭路径变化时，复用正常 300 秒采集的硬件事实并由 T-P1-IPC、T-P3-RECOVER 和 5.6 补充生命周期证据。
