...
- F0-A 的吞吐结果只作为块格式和 300 秒容量输入；生产 10×32-bit、1 kHz、300 秒持久化行为已由 `validation/p3-stage.md` 真机纵向验收闭环。
- Capture Store 版本、块头、CRC/SHA、终态清单、路径、大小估算、IPC 上限字段或 Worker 恢复路径变化时，本证据对应部分失效。
- 4.6 在同一版本终态清单中增加质量元数据后，已重新运行 T-P3-STORE；CRC、SHA、发布前校验、原子重命名、只读重开与临时恢复断言继续通过。
- 4.7 按 HSSA-009 将完整 `TargetConnectionSpec` 加入自描述头并用于恢复身份校验后，再次运行 T-P3-STORE；完成文件枚举、头/清单身份一致性、目标身份重开和部分文件保留断言继续通过。该内部格式在 V1 发布前收敛，不扩大公共 MCP Schema。
- 父进程有限续行和查询/资源接口分别由 4.7、5.1–5.5 完成；本任务不声明这些能力已完成。
