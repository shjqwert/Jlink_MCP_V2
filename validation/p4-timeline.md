...
- 首次页返回当页所需字典；续页仅返回新出现的 series 增量。changes、window raw/aggregates 和 around_event 共用同一续页入口，调用方不能通过游标请求修改范围、规则、mode、points、limit 或排序。
...
- `crates/jlink-mcp/tests/t_p4_timeline.rs`
  ...
  - raw window limit=2 精确续接第三条重复保留行，后页字典为空；around_event limit=1 续接第二条附近变化。
