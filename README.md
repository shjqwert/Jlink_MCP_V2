...
当前源码版本为 **1.1.5**。本补丁同步任务授权和最小验证规则，保留设备独占及硬件安全边界。以下为 1.1.4 已有功能与历史验证，本次补丁验证另行记录。

1.1.4 修复写后验证错误、Worker 超时与 HSS 清理隔离，支持中断采集有效前缀查询和导出；完善离线规划、配置更新、速率评估复用及有证据的增量构建流程。
